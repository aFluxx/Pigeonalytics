...
<form action="{{ route('race.update', $race->id) }}" enctype="multipart/form-data" method="POST"
    ...
    <select name="race_wind_strength" id="race_wind_strength" class="tw-input">
        ...
        <option value="light_breeze" {{($race->wind_strength ==='light_breeze') ? 'selected' : ''}}>Light breeze (licht)
        </option>
        <option value="gentle_breeze" {{($race->wind_strength ==='gentle_breeze') ? 'selected' : ''}}>Gentle breeze
            (matig)</option>
        <option value="strong_wind" {{($race->wind_strength ==='strong_wind') ? 'selected' : ''}}>Strong wind</option>
        <option value="storm_wind" {{($race->wind_strength ==='storm_wind') ? 'selected' : ''}}>Storm wind</option>
    </select>
    ...
    <select name="race_overcast" id="race_overcast" class="tw-input">
        ...
        <option value="very_light_clouds" {{($race->overcast ==='very_light_clouds') ? 'selected' : ''}}>Very light
            clouds</option>
        <option value="light_clouds" {{($race->overcast ==='light_clouds') ? 'selected' : ''}}>Light clouds</option>
        <option value="moderate_clouds" {{($race->overcast ==='moderate_clouds') ? 'selected' : ''}}>Moderate clouds
        </option>
        <option value="heavy_clouds" {{($race->overcast ==='heavy_clouds') ? 'selected' : ''}}>Heavy clouds</option>
        <option value="very_heavy_clouds" {{($race->overcast ==='very_heavy_clouds') ? 'selected' : ''}}>Very heavy
            clouds</option>
    </select>
    ...
    <select name="race_rainfall" id="race_rainfall" class="tw-input">
        ...
        <option value="very_light_rain" {{($race->rainfall ==='very_light_rain') ? 'selected' : ''}}>Very light rain
        </option>
        <option value="light_rain" {{($race->rainfall ==='light_rain') ? 'selected' : ''}}>Light rain</option>
        <option value="moderate_rain" {{($race->rainfall ==='moderate_rain') ? 'selected' : ''}}>Moderate rain</option>
        <option value="heavy_rain" {{($race->rainfall ==='heavy_rain') ? 'selected' : ''}}>Heavy rain</option>
        <option value="very_heavy_rain" {{($race->rainfall ==='very_heavy_rain') ? 'selected' : ''}}>Very heavy rain
        </option>
    ...
    <input type="number" id="race_amount_pigeons" name="race_amount_pigeons" class="tw-input"
        value="{{ old('race_amount_pigeons', $race->amount_of_pigeons) }}" />
