...
@section('content')
<div class="p-4">
    <h2 class="tw-font-bold tw-text-2xl tw-mb-2">Bewerk race</h2>
    <form action="{{ route('race.update', $race->id) }}" enctype="multipart/form-data" method="POST"
        ...
        <div class="tw-flex">
            <div class="tw-w-1/2 tw-pr-2">
                ...
                <input type="datetime-local" name="race_unloading_time" id="race_unloading_time" step="1"
                    class="tw-input"
                ...
                <select name="race_type" id="race_type" class="tw-input">
                    ...
                    <option value="competition" {{($race->type ==='competition') ? 'selected' : ''}}>Competition
                    </option>
                </select>
                ...
                <select name="race_overcast" id="race_overcast" class="tw-input">
                    ...
                    <option value="very_light_clouds" {{($race->overcast ==='very_light_clouds') ? 'selected' : ''}}>
                        Very
                        light
                        clouds</option>
                    <option value="light_clouds" {{($race->overcast ==='light_clouds') ? 'selected' : ''}}>Light clouds
                    </option>
                    <option value="moderate_clouds" {{($race->overcast ==='moderate_clouds') ? 'selected' : ''}}>
                        Moderate
                        clouds
                    </option>
                    <option value="heavy_clouds" {{($race->overcast ==='heavy_clouds') ? 'selected' : ''}}>Heavy clouds
                    </option>
                    <option value="very_heavy_clouds" {{($race->overcast ==='very_heavy_clouds') ? 'selected' : ''}}>
                        Very
                        heavy
                        clouds</option>
                </select>
                ...
                <select name="race_rainfall" id="race_rainfall" class="tw-input">
                    ...
                    <option value="very_light_rain" {{($race->rainfall ==='very_light_rain') ? 'selected' : ''}}>Very
                        light
                        rain
                    </option>
                    <option value="light_rain" {{($race->rainfall ==='light_rain') ? 'selected' : ''}}>Light rain
                    </option>
                    <option value="moderate_rain" {{($race->rainfall ==='moderate_rain') ? 'selected' : ''}}>Moderate
                        rain
                    </option>
                    <option value="heavy_rain" {{($race->rainfall ==='heavy_rain') ? 'selected' : ''}}>Heavy rain
                    </option>
                    <option value="very_heavy_rain" {{($race->rainfall ==='very_heavy_rain') ? 'selected' : ''}}>Very
                        heavy
                        rain</option>
                    <option value="downpour_hail" {{($race->rainfall ==='downpour_hail') ? 'selected' : ''}}>Downpour /
                        Hail
                    </option>
                </select>
            </div>
            ...
            <div>
                ...
                <div id="fon_races" class="tw-hidden">
                    ...
                    <label for="race_amount_of_pigeons_provincial" class="tw-label">Amount of pigeons
                        (Provincial):</label>
